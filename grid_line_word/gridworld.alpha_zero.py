...
def choose_action_with_neural_mcts(env: DeepSingleAgentEnv,
                                   model: tf.keras.Model,
                                   iterations_count: int = 200) -> np.array([int, np.ndarray]):
    tree = {}
...
def naive_alpha_zero(env: DeepSingleAgentEnv,
                     model: tf.keras.Model,
                     games_count: int,
...
def create_model(env: DeepSingleAgentEnv):
...
def train_model(env: DeepSingleAgentEnv, X_train, Y_train, epochs: int = 10000) -> tf.keras.models.Model:
    # create model
    pi_input_state_desc = tf.keras.layers.Input((env.state_dim(),))
    pi_input_mask = tf.keras.layers.Input((env.max_action_count(),))
...
def alpha_zero(training_games_count: int, games_count: int, env):
...
alpha_zero(1000, 1000, LineWorld(10))
#alpha_zero(1000, 1000, GridWorld(5,5))