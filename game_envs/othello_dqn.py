...
def dqn(env: DeepSingleAgentEnv, max_iter_count: int = 10000,
    ...
    q.add(tf.keras.layers.Dense(128,activation=tf.keras.activations.relu,
                                use_bias=True
                                ))
    q.add(tf.keras.layers.Dropout(0.2))
    ...
    q_target.add(tf.keras.layers.Input((env.state_dim(), )))
    q_target.add(tf.keras.layers.Dense(128,activation=tf.keras.activations.relu,
                                use_bias=True
                                ))
    q_target.add(tf.keras.layers.Dropout(0.2))
    q_target.add(tf.keras.layers.Dense(env.max_action_count(),
                                activation=tf.keras.activations.linear,
                                use_bias=True
                                ))
...
q_model, scores, steps = dqn(OthelloEnv(), epsilon=0.1, max_iter_count=10000)
print(q_model.weights)
q_model.save(os.path.join('models',"Othello", "dqn.h5"))
plt.plot(scores)
plt.show()
plt.plot(steps)
plt.show()
