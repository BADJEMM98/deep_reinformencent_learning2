...
class InterleavedAgent(Agent):
    ...
    def forward(self, observation):
        ...
        """
        # Determine current player from observation
        self.cur_agent = observation[0]
        # Pass observation to current agent
        return self.agents[self.cur_agent].forward(observation[1])
    ...
    def backward(self, reward, terminal):
        ...
        """
        return self.agents[self.cur_agent]\
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        # Truncate the list of metrics if one has more (assume prefixes match)
        if len(self.agents[0].metrics_names) <= \
                len(self.agents[1].metrics_names):
